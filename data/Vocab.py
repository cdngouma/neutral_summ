...
class Vocab(object):
    ...
    def word2id(self, word):
        ...
        unk_id = self.unk()
        word_id = self._word_to_id.get(word, unk_id)
        return word_id
    
    #####Error here - to return proper word change if condition 
    #if word_id is not in 0 to length of list print error
    #otherwise get elem of list self._id_to_word[word_id]
    def id2word(self, word_id):
        """Returns the word (string) corresponding to an id (integer)."""
        if word_id not in range(len(self._id_to_word)):
    ...
    def outputids2words(self, ids, src_oovs=None):
        """Maps output ids to words
        Args:
            ids: list of ids
            src_oovs: list of oov words or None
        Returns:
        ...
        for i in ids:
            try:
                ...
                try:
                    ...
                    raise ValueError(f'Error: model produced word ID {i} but this example only has {len(src_oovs)} article OOVs')
            words.append(w)
        return words
